...
class MovieAppRepository private constructor(
    ...
    private val appExecutors: AppExecutors): MovieAppDataSource{

    companion object{
        @Volatile
        private var instance: MovieAppRepository? = null

        fun getInstance(remoteData: RemoteDataSource, localData: LocalDataSource, appExecutors: AppExecutors): MovieAppRepository =
            instance ?: synchronized(this){
                instance ?: MovieAppRepository(remoteData, localData, appExecutors).apply { instance = this }
            }
    }
    ...
    override fun getAllSeries(): LiveData<Resource<PagedList<SeriesEntity>>>{
        return object : NetworkBoundResource<PagedList<SeriesEntity>, List<SeriesResponse>>(appExecutors){
            ...
            override fun saveCallResult(data: List<SeriesResponse>) {
                ...
                for (response in data){
                    val serie = SeriesEntity(response.id,
                        response.title,
                        response.description,
                        response.year,
                        response.imagePath,
